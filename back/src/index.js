...
//// Routs for gallery_images:
  ...
  app.post("/users/update/:id", async (req, res, next) => {
    ...
    try {
      ...
      const result = await controller_users.updateUsers(id, {first_name, last_name, email, password, phone_number, address, postal_code, city});
      res.json({ success: true, result });
      return (result);
    } catch (err) {
      next(err);
  ...
  app.post("/users/create", async (req, res, next) => {
    try {
      const {first_name, last_name, email, password, phone_number, address, postal_code, city} = req.query;
      const result = await controller_users.createUsers({first_name, last_name, email, password, phone_number, address, postal_code, city});
      res.json({ success: true, result });
    } catch (err) {
      next(err);
    }
